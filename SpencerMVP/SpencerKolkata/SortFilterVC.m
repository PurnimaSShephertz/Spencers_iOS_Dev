...
@interface SortFilterVC ()
{
    ...
    NSUserDefaults * temp;
    int globalTag;
    NSMutableArray *quantityIdArr, *brandIdArr;
    int sortingByIndex;
    ProductVC *productVC;
}
@end

@implementation SortFilterVC
...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    sortingByIndex = (int)[[[NSUserDefaults standardUserDefaults] valueForKey:@"sortingByIndex"] integerValue];
    
    [self sortByBtnAct:sortingByIndex];
    appDele = (AppDelegate *)[[UIApplication sharedApplication] delegate];
    
    
    NSArray *brandTempArr = [[NSUserDefaults standardUserDefaults] valueForKey:@"brandIdMyArr"];
    NSArray *quantityTempArr = [[NSUserDefaults standardUserDefaults] valueForKey:@"quantityIdMyArr"];
    ...
    sortFilterDict = appDele.filterDict;
    
    
    //    appDele.brandIdArr = brandIdArr;
    //    appDele.quantityIdArr = quantityIdArr;
    ...
    brandSortedArray = [[NSMutableArray alloc] init];
    quantitySortedArray = [[NSMutableArray alloc] init];
    
    
    for (int i = 0; i < tempBrandKeyArr.count; i++ )
    {
        NSDictionary *tempDict = [[sortFilterDict valueForKey:[filterHeaderArr objectAtIndex:0]] valueForKey:[tempBrandKeyArr objectAtIndex:i]];
        NSMutableDictionary *brandDict = [[NSMutableDictionary alloc] init];
        [brandDict setObject:[tempBrandKeyArr objectAtIndex:i] forKey:@"id"];
        [brandDict setObject:[tempDict valueForKey:@"isset"] forKey:@"isset"];
        [brandDict setObject:[tempDict valueForKey:@"label"] forKey:@"label"];
        
//        NSLog(@"brand Dict %@", brandDict);
        [brandSortedArray addObject:brandDict];
    }
    
    for (int i = 0; i < tempQuantityKeyArr.count; i++ )
    {
        NSDictionary *tempDict = [[sortFilterDict valueForKey:[filterHeaderArr objectAtIndex:1]] valueForKey:[tempQuantityKeyArr objectAtIndex:i]];
        NSMutableDictionary *brandDict = [[NSMutableDictionary alloc] init];
        [brandDict setObject:[tempQuantityKeyArr objectAtIndex:i] forKey:@"id"];
        [brandDict setObject:[tempDict valueForKey:@"isset"] forKey:@"isset"];
        [brandDict setObject:[tempDict valueForKey:@"label"] forKey:@"label"];
        
//        NSLog(@"brand Dict %@", brandDict);
        [quantitySortedArray addObject:brandDict];
    }
    
    NSSortDescriptor *distanceSortDiscriptor = [NSSortDescriptor sortDescriptorWithKey:@"label"
                                                                             ascending:YES
                                                                              selector:@selector(localizedStandardCompare:)];
    


    brandKeyArr    = [brandSortedArray sortedArrayUsingDescriptors: [NSArray arrayWithObject: distanceSortDiscriptor]];
    quantityKeyArr = [quantitySortedArray sortedArrayUsingDescriptors: [NSArray arrayWithObject: distanceSortDiscriptor]];
...
//    NSString *finalDate = [dateFormatter stringFromDate:yourDate];
    ...
    [btn setTitle:[NSString stringWithFormat:@"%@",
                   [[filterHeaderArr objectAtIndex:section] capitalizedString]] forState:UIControlStateNormal];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        cell.textLabel.text = [NSString stringWithFormat:@"%@",[[brandKeyArr objectAtIndex:indexPath.row] valueForKey:@"label"]];
        
        BOOL temp = NO;
        
        NSString *brandIdMainStr = [[brandKeyArr objectAtIndex:indexPath.row] valueForKey:@"id"];
...
//        }
    ...
    {
        cell.textLabel.text = [NSString stringWithFormat:@"%@",[[quantityKeyArr objectAtIndex:indexPath.row] valueForKey:@"label"]];
        
        BOOL temp = NO;
        NSString *brandIdMainStr = [[quantityKeyArr objectAtIndex:indexPath.row] valueForKey:@"id"];
...
//        for (int i = 0; i < brandKeyArr.count; i++)
//        {
        
            BOOL exist = NO;
            for (int  j = 0 ; j < brandIdArr.count; j++ )
            {
                if ([[[brandKeyArr objectAtIndex:indexPath.row] valueForKey:@"id"] isEqualToString:[brandIdArr objectAtIndex:j]])
                {
                    exist = YES;
                    [brandIdArr removeObject:[[brandKeyArr objectAtIndex:indexPath.row] valueForKey:@"id"]];
                    break;
                }
            }
            if (exist == NO)
            {
                [brandIdArr addObject:[[brandKeyArr objectAtIndex:indexPath.row] valueForKey:@"id"]];
            }
...
//        }
        ...
        for (int j = 0; j < brandIdArr.count; j++ )
        {
            if (j == 0)
            {
                [brandSeletedStr appendFormat:@"%@", [brandIdArr objectAtIndex:j]];
            }
            else
            {
                [brandSeletedStr appendFormat:@"-%@", [brandIdArr objectAtIndex:j]];
...
//        brandSeletedStr = [brandKeyArr objectAtIndex:indexPath.row];
    ...
    else if (selectedSection == 2)
    {
        ...
        {
            if ([[[quantityKeyArr objectAtIndex:indexPath.row] valueForKey:@"id"] isEqualToString:[quantityIdArr objectAtIndex:j]])
            {
                exist = YES;
                [quantityIdArr removeObject:[[quantityKeyArr objectAtIndex:indexPath.row] valueForKey:@"id"]];
                break;
            }
        }
        if (exist == NO)
        {
            [quantityIdArr addObject:[[quantityKeyArr objectAtIndex:indexPath.row] valueForKey:@"id"]];
        }
...
//        for (int i = 0; i < quantityIdArr.count; i++)
//        {
//            BOOL exist = NO;
//            for (int  j = 0 ; j < quantityKeyArr.count; j++ )
//            {
//                if ([[quantityIdArr objectAtIndex:i] isEqualToString:[quantityKeyArr objectAtIndex:j]])
//                {
//                    exist = YES;
//                    [quantityIdArr removeObject:[quantityKeyArr objectAtIndex:[[section2SelectionArr objectAtIndex:i] integerValue]]];
//                    break;
//                }
//            }
//            if (exist == NO)
...
////                
////                [quantityIdArr removeObject:[quantityKeyArr objectAtIndex:[[section2SelectionArr objectAtIndex:i] integerValue]]];
////                [section2SelectionArr removeObjectAtIndex:i];
////                
////                break;
////            }
//            
...
//        if (exist == NO)
//        {
//            [section2SelectionArr addObject:[NSString stringWithFormat:@"%li", (long)indexPath.row]];
...
//        }
        ...
        for (int j = 0; j < quantityIdArr.count; j++ )
        {
            if (j == 0)
            {
                [quantitySelectedStr appendFormat:@"%@", [quantityIdArr objectAtIndex:j]];
            }
            else
            {
                [quantitySelectedStr appendFormat:@"-%@", [quantityIdArr objectAtIndex:j]];
...
//    appDele.quantityIdArr = quantityIdArr;
    ...
    [[NSUserDefaults standardUserDefaults] setObject:brandIdArr forKey:@"brandIdMyArr"];
    [[NSUserDefaults standardUserDefaults] setObject:quantityIdArr forKey:@"quantityIdMyArr"];
...
- (IBAction)sortBtnAct:(UIButton *)sender
{
    ...
    [[NSUserDefaults standardUserDefaults] setInteger:globalTag forKey:@"sortingByIndex"];
