...
static CGFloat kboardHeight = 244.0f;
...
#endif
		kboardHeight = 244.0f + keyBoardToolbarHeight;
	}
...
- (void)animateView:(BOOL)isShow textField:(id)textField heightforkeyboard:(CGFloat)kheight
{
    [[NSUserDefaults standardUserDefaults]setBool:YES forKey:@"checkkeyboard"];
    
	kboardHeight = kheight;
	[self checkBarButton:textField];
	 rect = objectView.frame;
	...
	if (isShow)
    {
		if ([textField isKindOfClass:[UITextField class]]) {
            ...
            {
                rect.origin.y = rect.size.height - textPoint.y - kheight + viewFrameY-40;
            }
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string
{
    ...
    if ([[NSUserDefaults standardUserDefaults]boolForKey:@"LoginCheck"]==YES)
    {
    ...
    else if ([[NSUserDefaults standardUserDefaults]boolForKey:@"registrationpagecheck"]==YES)
    {
   ...
   else if ([[NSUserDefaults standardUserDefaults]boolForKey:@"profilecheck"]==YES)
    {
   ...
   else if ([[NSUserDefaults standardUserDefaults]boolForKey:@"Addaddress"]==YES)
   {
   ...
   else if ([[NSUserDefaults standardUserDefaults]boolForKey:@"Pincheck"]==YES)
   {
       if(textField.tag==0)
       {
           NSUInteger newLength = [textField.text length] + [string length] - range.length;
