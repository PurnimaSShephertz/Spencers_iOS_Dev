...
{
    if (appDele.pushFlag == 21)
    {
        [self.navigationController popViewControllerAnimated:YES];
    }
    else if (appDele.pushFlag == 20)
    {
        BOOL flag = NO;
...
//    UIPickerView *pickerView = [[UIPickerView alloc] init];
    ...
    self.amount = appDele.citrusfinalamount;
    ...
    if (self.landingScreen==3) {
        
        self.title = @"Net Banking";
        NSString *string = [NSString stringWithFormat:@"Pay Rs. %@",self.amount];
        [self.loadButton setTitle:string forState:UIControlStateNormal];
        //        [self.loadMoneyButton setTitle:@"Payment" forState:UIControlStateNormal];
...
-(IBAction)loadUsingCard:(id)sender{
    ...
    if ([appDele.Checkpage isEqualToString:@"11"] || [appDele.Checkpage isEqualToString:@"12"]) {
        
        self.title = appDele.pageType;
        [self.saveCardsTableView setHidden:TRUE];
    ...
    else if ([appDele.Checkpage isEqualToString:@"13"]){
...
//        globalInt = 13;
        self.landingScreen = appDele.creditCardNetbankingFlag;
        
    }
    else if ([appDele.Checkpage isEqualToString:@"14"]){
...
//            }
    ...
    if (self.landingScreen == 1 || self.landingScreen == 3) {
        
        [CTSUtility requestBillAmount:self.amount billURL:[NSString stringWithFormat:@"%@?orderid=%@&amount=%@&os=ios", BillUrl, orderIdStr, self.amount] callback: ^(CTSBill *bill , NSError *error)
        {
...
//                     
//                 }
                 if ([[NSString stringWithFormat:@"%@",[citrusCashResponse.responseDict valueForKey:@"pgRespCode"]] isEqualToString:@"0"])
                 {
...
//                     [alertView show];
        ...
        [paymentLayer requestLoadMoneyInCitrusPay:cardInfo withContact:contactInfo withAddress:addressInfo amount:self.amount returnUrl:[NSString stringWithFormat:@"%@?orderid=%@&amount=%@&os=ios", LoadWalletReturnUrl, orderIdStr, self.amount] customParams:customParams  returnViewController:self withCompletionHandler:^(CTSCitrusCashRes *citrusCashResponse, NSError *error) {
...
//    [paymentInfo addCard:instrument];
    ...
    [paymentLayer requestPerformDynamicPricingRule:self.ruleInfo paymentInfo:cardInfo billUrl:[NSString stringWithFormat:@"%@?orderid=%@&amount=%@&os=ios", BillUrl, orderIdStr, self.amount] user:user extraParams:nil completionHandler:^(CTSDyPResponse *dyPResponse, NSError *error) {
        dispatch_async(dispatch_get_main_queue(), ^{
            [self.indicatorView stopAnimating];
            self.indicatorView.hidden = TRUE;
        });
...
- (void) setCardInfo{
    ...
    if ([appDele.Checkpage isEqualToString:@"13"])
    {
